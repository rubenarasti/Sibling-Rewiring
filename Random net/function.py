...
def solve(G_siblings, percentage_component, percentage_individual, totalStudents):
    """
...
def linear_cooling_sequence(t, it):
    """
...
def geometrical_cooling_sequence(values, t):
    """
...
def  logarithmic_cooling_sequence(initial_t, it, values):
    """
...
def solve_simulated_annealing(G, matrix, siblings, totalStudents, l, tf, values, option_cooling, seed_value):
    """
    ...
    initial_t = solve(G, percentage_component, percentage_individual, totalStudents) * 0.4
    t = initial_t
    current_solution = G
    it = 0
    ini_fmax = solve(G, percentage_component, percentage_individual, totalStudents)
    
    while t >= tf:
        for i in range(l):
            candidate_solution = generate_neighbor(matrix,current_solution, siblings, seed_value)
            
            candidate_fmax = solve(candidate_solution, percentage_component, percentage_individual, totalStudents)
            current_fmax = solve(current_solution, percentage_component, percentage_individual, totalStudents)
        ...
        if option_cooling == 1:        
            t = linear_cooling_sequence(t, it)
        elif option_cooling == 3:
            t = logarithmic_cooling_sequence(initial_t, it, values)
        elif option_cooling == 4:
            t = cauchy_cooling_sequence(initial_t, it)
        elif option_cooling == 5:
            t = modified_cauchy_cooling_sequence(initial_t, tf, l, t)
        elif option_cooling == 6:
            t = geometrical_cooling_sequence(values, t)
